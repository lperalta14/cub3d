...
int	read_line_and_parse(t_scene *data, char *line)
{
	...
	if (!ft_strncmp("NO", line + i, 2))
		return (in_data_texture(line + i + 2, &data->texture.n));
	if (!ft_strncmp("SO", line + i, 2))
		return (in_data_texture(line + i + 2, &data->texture.s));
	if (!ft_strncmp("WE", line + i, 2))
		return (in_data_texture(line + 2 + i, &data->texture.w));
	if (!ft_strncmp("EA", line + i, 2))
		return (in_data_texture(line + 2 + i, &data->texture.e));
